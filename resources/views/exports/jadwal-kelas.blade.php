...
<head>
    ...
    <style>
        ...
        .mapel { font-weight: bold; color: #0f172a; margin-bottom: 4px; display: block; font-size: 11px; }
...
@foreach($entities as $index => $kelas)
    @if($index > 0)
    ...
    <div class="title">Jadwal Kelas: {{ $kelas->nama }}</div>
    ...
    <table>
        ...
        <tbody>
            @php
                // Cari jumlah jam maksimal dalam seminggu
                $maxJam = 0;
                foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $hari) {
            ...
            @for($i = 0; $i < $maxJam; $i++)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td>
                        {{-- Mengambil waktu dari hari Senin sebagai acuan rentang waktu (atau hari pertama yang ada) --}}
                        @php
                            $waktuAcuan = '-';
                            foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $hari) {
                    ...
                    @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $hari)
                        ...
                        @if($jam)
                            ...
                            @else
                                <td>
                                    @if(isset($jadwalGrouped[$kelas->id][$hari][$jam->id]))
                                        @foreach($jadwalGrouped[$kelas->id][$hari][$jam->id] as $j)
                                            <div style="{{ !$loop->last ? 'margin-bottom: 5px; border-bottom: 1px dashed #cbd5e1; padding-bottom: 5px;' : '' }}">
                                                <span class="mapel">{{ $j->mapel->nama }}</span>
